...
int i_terminated = 0;
//Questa volta il numero di celle assegnate deve essere una variabile globale perchè mi serve sapere nella
//funzione solve quali tabelle sono vincolate
...
int print_matrix(block** matrix){
    ...
    for(int i = 0; i < size; i++) {
        for(int j = 0; j < size; j++) {
            	...
            	printf("%hi\t", matrix[i][j].value);
						else if(matrix[i][j].state == 'b')
	            printf("x\t");
						else
							printf("%i,%c\t", matrix[i][j].value, matrix[i][j].state);
        }
    printf("\n");
    }
    return 1;
